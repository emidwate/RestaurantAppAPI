...
to access the app. The default TCP/IP port is 3306. The default username is ```root```, and the default password 
is also set to ```root```, if you want to change these values go to ```application.properties``` file and do so, 
the only requirement is to have ```java``` installed on your machine.
